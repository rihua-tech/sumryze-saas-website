...
import DOMPurify from "isomorphic-dompurify"; // ✅ default export is the instance

type AISummaryCardProps = { showActions?: boolean };
...
export default function AISummaryCard({ showActions = true }: AISummaryCardProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/ai-summary?url=${encodeURIComponent(currentUrl)}`, { cache: "no-store" });
  ...
  // ensure string, then sanitize the highlighted HTML
  return DOMPurify.sanitize(applyColorHighlighting(String(summary ?? "")));
}, [summary]);
  ...
  const handleShare = async () => {
    if (navigator.share) {
      try {
        await navigator.share({ title: "SEO Summary", text: summary, url: window.location.href });
      } catch {
      ...
      <div className="bg-gradient-to-r from-purple-700 to-blue-600 text-white rounded-2xl p-6 shadow-md w-full">
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          ...
          <div className="w-full md:w-3/4">
            ...
            <div className="text-base leading-relaxed" dangerouslySetInnerHTML={{ __html: safeHtml }} />
          </div>
