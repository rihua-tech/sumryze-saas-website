...
import { ArrowDown, ArrowUp, CheckCircle2, AlertTriangle, XCircle, FileText, Info } from "lucide-react";
...
function StatusChip({ health, isDark }: { health: Health; isDark: boolean }) {
  const map = {
    healthy:   { cls: isDark ? "bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/15" : "bg-emerald-50 text-emerald-700 hover:bg-emerald-100", Icon: CheckCircle2, label: "Healthy" },
    watch:     { cls: isDark ? "bg-amber-500/10  text-amber-400  hover:bg-amber-500/15"   : "bg-amber-50   text-amber-700   hover:bg-amber-100",   Icon: AlertTriangle, label: "Watch" },
    declining: { cls: isDark ? "bg-rose-500/10   text-rose-400   hover:bg-rose-500/15"    : "bg-rose-50    text-rose-700    hover:bg-rose-100",    Icon: XCircle,       label: "Declining" },
  } as const;
  const { cls, Icon, label } = map[health];
  return (
    <span className={clsx("inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-semibold whitespace-nowrap", cls)}>
...
/* ----- helpers (unchanged) ----- */
function smoothMA3(arr: number[]) { if (arr.length < 3) return arr.slice(); const out: number[] = []; for (let i = 0; i < arr.length; i++) { const a = arr[i - 1] ?? arr[i]; const b = arr[i]; const c = arr[i + 1] ?? arr[i]; out.push((a + b + c) / 3); } return out; }
const mean = (xs: number[]) => (xs.length ? xs.reduce((a, b) => a + b, 0) / xs.length : 0);
function classifyHealth(series?: number[]): { health: Health } {
  if (!series || series.length < 4) return { health: "watch" };
  const sm = smoothMA3(series); const half = Math.floor(sm.length / 2);
  const prevMean = Math.max(1, mean(sm.slice(0, half))); const last = sm[sm.length - 1];
  const pctChange = ((last - prevMean) / prevMean) * 100; const absDrop = prevMean - last;
  let declinePct = 10, minDropPages = 5;
  ...
  if (pctChange <= -declinePct && absDrop >= minDropPages) return { health: "declining" };
...
export default function TopPagesCard({
  ...
  const { health } = classifyHealth(series);
  const fallbackHealth: Health = typeof down === "boolean" ? (down ? "declining" : "healthy") : "watch";
  const finalHealth: Health = series && series.length >= 4 ? health : fallbackHealth;

  const chartColor =
    finalHealth === "healthy"
      ? "text-emerald-400 group-hover:text-emerald-500 transition-colors duration-200"
      : finalHealth === "watch"
      ? "text-amber-400 group-hover:text-amber-500 transition-colors duration-200"
      : "text-rose-400 group-hover:text-rose-500 transition-colors duration-200";

  // Neutral number tone (no health tint)
      ...
      >
        ...
        <div className="relative mb-2 sm:mb-3 min-w-0">
          <div className="flex items-center gap-2 min-w-0 pr-12 md:pr-14">
            ...
            {/* Don’t truncate on md (iPad); only prevent wrap on xs */}
            ...
            <Tooltip>
              ...
              <TooltipContent side="top" className="max-w-[280px]">
                <p className={clsx("text-xs", isDark ? "text-gray-200" : "text-slate-700")}>
                  Number of pages currently indexed. Health compares the latest value to the previous-period
                  average with smoothing and size-aware thresholds.
                </p>
              </TooltipContent>
            </Tooltip>
          </div>
        ...
        <div className="flex-1 flex flex-col gap-3">
          {/* Number + status */}
            ...
            <div className="flex items-center gap-x-3 gap-y-2 flex-wrap
                justify-center mt-4 md:justify-start text-center md:text-left">

            <div
              className={clsx(
                "font-extrabold leading-none tabular-nums tracking-tight whitespace-nowrap",
                "text-[clamp(2rem,5.2vw,3.25rem)]", // a touch smaller on md for balance
                numberTone,
                "opacity-95 group-hover:opacity-100 transition-opacity"
              )}
              aria-label={`Top pages count ${value}`}
            >
          ...
          <div className="mt-auto w-full h-10 sm:h-12 md:h-14">
            <TopPagesChart series={series} colorClass={chartColor} className="w-full h-full" />
          </div>
        </div>
      </div>
    </TooltipProvider>
